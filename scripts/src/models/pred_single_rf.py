...
# --- Validation split: prefer saved split from training
...
if valid_split_path.exists():
    ...
    if not valid_ids:
        raise SystemExit(
            "Saved validation split IDs do not match the current dataset. "
            "Delete the saved split files or regenerate training first."
        )

    test_data = data.loc[valid_ids].copy()
else:
    print("No predefined validation split found. Falling back to random_state=42 80/20 split.")
