    ...
    def trainMultiTargetRFModels(
            ...
            for i, target_column in enumerate(targets_df.columns):
                ...
                if trim_by_percentile and not is_class_like:
                    ...
                    common_idx = current_features_df.index.intersection(trimmed_target_df.index)
                ...
                inf_cols = [
                    ...
                    if np.any(~np.isfinite(combined_data[c].to_numpy(dtype=float, na_value=np.nan)))
                ]
                if inf_cols:
                    self.logger.warning(
                    ...
                    combined_data = combined_data.replace([np.inf, -np.inf], np.nan).dropna(subset=feature_cols)
                ...
                if expected_task_type in {"binary_classification", "multiclass_classification"}:
                ...
                self.logger.info(f"  unique values: {sorted(combined_data[target_column].dropna().unique())}")
                    ...
                    if "Importance" in fi_df.columns:
                        ...
                        if combined_feat_importance_csv.exists():
                            ...
                            combined_df = combined_df.loc[
                                :, ["Feature"] + [
                                    col for col in combined_df.columns
                                    if col.startswith("Importance_") and col not in stale_imp_cols
                                ]
                            ]
                            combined_df = combined_df.set_index("Feature")
                            fi_df = fi_df.set_index("Feature")
                            combined_df = combined_df.reindex(combined_df.index.union(fi_df.index))
