...
# %% ------------------- Feature Importance
feat_imp_df = paths["prediction_output_dirs"]["rf"]['bp']['rdkit'] / "feature_importance.csv"
...
vis.plotFeatureImportance(feature_importance_df, top_n=20)


# %% ------------------- Multi-task Performance
experiment = "pred_mordred_tr_chemberta"
...
group_performance_df = vis.computeGroupPerf(
    ...
    descriptor_groups=getGroups("mordred"),
    metrics=["Pearson_r", "r2", "RMSE", "Bias"],
)
# %%
df = multitask_performance_df["SDEP"]
df["SDEP"]
# %%

vis.plotGroupRadar(group_performance_df,
                title=f"Mordred Prediction (ChemBERTa trained)",
                   save_plot=True,
                   save_path=mt_perf_path,
                   save_fname=f"{experiment}_radar")

# %% ------------------- Comparing Group Performances
vis.plotGroupBar(group_performance_df, labels=["mordred"])

# %% -------------------  for cross-embedding predictions
emb_desc_keys = all_data["embedding_and_descriptor_predictions"].keys()

emb_desc_dfs = []
for k in emb_desc_keys:
    df_ = pd.read_csv(all_data["embedding_and_descriptor_predictions"][k], index_col=0)
...
perf_files = {
    "RDKit":   all_data["LD50_predictions_rf"]["rdkit_internal_performance"],
    "Mordred": all_data["LD50_predictions_rf"]["mordred_internal_performance"],
    "ChemBERTa": all_data["LD50_predictions_rf"]["chemberta_internal_performance"],
    "MolFormer": all_data["LD50_predictions_rf"]["molformer_internal_performance"],
}
